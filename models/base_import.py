...
class BSAImport(models.TransientModel):
    ...
    def add_attributes_and_values_3(self, db, uid, password, models, database_ids, attr_val_dict, product_field_information,fields,columns):
    ...
    def add_attributes_and_values(self, db, uid, password, models, database_ids, attr_val_dict, product_field_information, fields, columns):
        # db, uid, password, models, database_ids, attr_val_dict, product_field_information,fields,columns
        clean_df = pd.read_excel(BytesIO(self.clean_file), engine='openpyxl')
        clean_df.rename(columns = lambda x: x.strip().lower(), inplace=True)

        #Find column that matches to name field
        name_col = None
        for i in range(len(fields)):
            if fields[i].lower() == 'name':
                name_col = columns[i].lower()
                break
        ...
        for row in range(0, len(clean_df)):
            ...
            if not pd.isna(clean_df[name_col][row]):
                ...
                exists, curr_product_id = self.check_record_existence(models, db, uid, password, curr_product_name, 'product.template')
                if exists:
                    models.execute_kw(db, uid, password,'product.template','write',[[curr_product_id],self.parse_fields(clean_df, row, product_field_information, db, uid, password, models)])
                    
                    product_attribute_dict[curr_product_id] = self.parse_attributes(clean_df, row, name_col, database_ids)

                else:
                    product_batch.append(self.parse_fields(clean_df, row, product_field_information, db, uid, password, models))
                    attribute_batch.append(self.parse_attributes(clean_df, row, name_col, database_ids))

                    if len(product_batch) > BATCH_SIZE:
                        self.batch_create_products(db, uid, password, models, product_batch, attribute_batch, product_attribute_dict)

                        product_batch = []
                        attribute_batch = []
                
                #Leftover records in batch

        if len(product_batch) > 0:
            self.batch_create_products(db, uid, password, models, product_batch, attribute_batch, product_attribute_dict)


        attribute_line_batch = []
        counter = 0
            
        for product in product_attribute_dict.keys():
            
            if counter % 50 == 0:
                print(counter)
            counter += 1

            for attribute in product_attribute_dict[product].keys():
                attribute_values = product_attribute_dict[product][attribute]

                exists, attribute_line_id = self.check_attribute_line_existence(models, db, uid, password, database_ids[attribute], product)

                new_attr_line = {
                        'product_tmpl_id': product,
                        'attribute_id': database_ids[attribute],
                        'value_ids': attribute_values
                }

                if exists:
                    models.execute_kw(db, uid, password, 'product.template.attribute.line', 'write', [[attribute_line_id], new_attr_line])
                else:
                    attribute_line_batch.append(new_attr_line)
                    if len(attribute_line_batch) > BATCH_SIZE:
                        models.execute_kw(db, uid, password, 'product.template.attribute.line', 'create', [attribute_line_batch])
                        attribute_line_batch = []

        if len(attribute_line_batch) > 0:
            models.execute_kw(db, uid, password, 'product.template.attribute.line', 'create', [attribute_line_batch])
        
    
    def parse_fields(self, df, row, product_field_information, db, uid, password, models):
        product_fields = df.iloc[row].to_numpy().tolist()
        columns = df.columns
        new_product_fields = {}

        for col in columns:
            if col != 'attribute' and col != 'value':
                field_name = product_field_information[col]['name']
                field_type = product_field_information[col]['type']
                field_val = df[col][row]
        ...
        return new_product_fields


    def parse_attributes(self, df, starting_row, name_col, database_ids):
        newProduct = True
        curr_product_attributes = {}
        print('starting row is ', starting_row)

        for row in range(starting_row, len(df)):
            if not pd.isna(df[name_col][row]):
                newProduct = not newProduct
                if newProduct:
                    break

            attribute = df['attribute'][row]
            values = df['value'][row].split(',')

            for i in range(len(values)):
                values[i] = (4, database_ids[values[i]], 0)

            curr_product_attributes[attribute] = values
        print('curr attributes are', curr_product_attributes)
        return curr_product_attributes

            
    def check_record_existence(self, models, db, uid, password, curr_item_name, record_model):
        item_record = models.execute_kw(db,uid,password, record_model,'search_read',[[['name','=',curr_item_name]]], {'fields': ['id']})
        if len(item_record) > 0:
            return True, item_record[0]['id']
        else:
            return False, None
    
    def check_attribute_line_existence(self, models, db, uid, password, attr_id, product_id):


        item_record = models.execute_kw(db, uid, password, 'product.template.attribute.line', 'search_read', [[['attribute_id', '=', attr_id], ['product_tmpl_id', '=', product_id]]], {'fields': ['id']})
        if len(item_record) > 0:
            return True, item_record[0]['id']
        else:
            return False, None
    ...
    def link_field_to_model(self, db, uid, password, models, field_val, comodel):
        ...
        for record in existing_model_records:
            name = record['name'][1]
            print('\n \n \n name is ', name)
            if name.lower() == str(field_val).lower():
    ...
    def batch_create_products(self, db, uid, password, models, parent_model_batch, attribute_batch, product_attribute_dict):
        ...
        for i in range(len(product_db_ids)):
            curr_product_id = product_db_ids[i]
            curr_product_attributes = attribute_batch[i]
            product_attribute_dict[curr_product_id] = curr_product_attributes 
...
class DirtyToAttributeValues():
    ...
    def get_attribute_names(self, input_array, all_columns):
        ...
        :param list all_columns: List of all columns in a dataframe
